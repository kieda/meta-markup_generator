...
public class SharedChars {
    ...
    /**
     * 
     * @param data the data that will be the super-sequence of all sequences in
     * this char array.
     */
    public SharedChars(final char[] data){this.data=data;this.length=data.length;}
    ...
    /**
     * A CharSequence that is directly backed by char[] data.
     * this will not explicitely throw any errors if a person passes in 
     * incorrect information
     */
    private final class Chars implements CharSequence{
        private final int pos, len;
        private Chars(final int pos, final int len){this.pos=pos; this.len=len;}
        @Override public final int length() {
            return len;
        }
        @Override public final char charAt(int index) {
            if(index<0||index>=pos+len) throw new IndexOutOfBoundsException("index = "+index+", len = "+len);
            return data[index+pos];
        }
        @Override public final CharSequence subSequence(int start, int end) {
            if(start<0||end>len||end<start) throw new IndexOutOfBoundsException("start = "+start+", end = "+end+", len = "+len);
            return new Chars(start+pos, end-start);
        }
        private String s =null;
        public String toString(){
            if(s==null) s=new String(data, pos, len);
            return s;
        }
    }
}
