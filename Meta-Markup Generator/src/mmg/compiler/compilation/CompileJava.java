...
import mmg.args.Flags;
import mmg.gen.JavaGen;
import mmg.util.io.SaveFile;
import java.io.File;
import java.util.regex.Pattern;
import mmg.lib.java_compile.InMemoryCompile;

/**
 * CompileJava takes a JavaGen and some compilation arguments. The JavaGen 
 * contains the information necessary to generate a java class, and the 
 * args provide information on how the file is compiled, what parts are saved,
 * and what location the class is compiled and saved.
 * 
 * @author zkieda
 */
public class CompileJava {
    ...
    public static void compile(JavaGen jclass, CompileArgs args) {
        ...
        if(args.flag(CompileArgs.IN_MEMORY_COMPILE)){
            try {
                InMemoryCompile.compile(jclass.getFullName().toString(), chars);
