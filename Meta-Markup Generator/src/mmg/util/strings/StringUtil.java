...
/**
 * is a class which contains a collection of commonly used or useful algorithms
 * on strings. 
 * 
 * @author zkieda
 */
public class StringUtil {
     ...
     * 
     * @param s the CharSequence we are finding the line from.
     * @param idx the index of the charsequence
     * @return the line number of the char at index idx in s
     * since this is mainly meant for error output, we start from line 1 (and 
     * not line 0).
     * 
     */
     ...
     * indices.length-1
     * 
     * @param s the charsequence we are analyzing. Must start at index 0.
     * @param indices the sequential list of indices of s we want to find the
     * line numbers of
     * @return a list of line numbers, such that the i^th line number from 
     * result represents the line number from the i^th index from indices.
     * 
     * throws NullPointerException if indices or s is null
     * throws InvalidArgumentException if indices is not ordered.
     * may throw an IndexOutOfBoundsException iff s throws it for indices out
     * of bounds.
     * 
    ...
    public static int[] getLineNum(CharSequence s, int[] indices) {
        ...
        for(int i = 0; i < indices.length; i++){
            if(cur_idx>(cur_idx = indices[i])) throw new IllegalArgumentException("indices are not well ordered, at index "+(i-1));
